...
class CombinedActorInfo {
    ...
    static async FromArrayLike(arrayLike) {
    ...
    static async FromArrayBuffer(buffer) {
        ...
        for (let index = 0; index < cai.entryCount; index++) {
            ...
            offset += 128;
            var actor = new Actor_1.Actor(name);
            actor.PrimaryMatrix = new ActorMatrix_1.ActorMatrix(primaryMatrix);
            actor.SecondaryMatrix = new ActorMatrix_1.ActorMatrix(secondaryMatrix);
